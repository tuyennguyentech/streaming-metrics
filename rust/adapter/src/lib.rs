...
async fn check_content_encoding(header_map: &HeaderMap) -> Result<&str, Response> {
...
async fn check_content_type(header_map: &HeaderMap) -> Result<&str, Response> {
  match header_map.get("Content-Type") {
    Some(value) if value == "application/x-protobuf;proto=io.prometheus.write.v2.Request" => {
      Ok("application/x-protobuf;proto=io.prometheus.write.v2.Request")
    }
    Some(value) if value != "" => Err(
      (
        StatusCode::UNSUPPORTED_MEDIA_TYPE,
        format!("Unknown remote write content type: {:?}", value),
      )
        .into_response(),
    ),
    _ => {
...
async fn decompress_body(body: Bytes) -> Result<Vec<u8>, Response> {
...
async fn handler_receive_prometheus_remote_write(header_map: HeaderMap, body: Bytes) -> Response {
  // tracing::debug!(
  //   "method = {} \nheader = {:#?}\n body = {:?}",
  //   method,
  //   header_map,
  ...
  let _content_encoding = match check_content_encoding(&header_map).await {
    Ok(value) => value,
    Err(res) => return res,
  };
  ...
  for timeseries in request.timeseries {
    println!(
      "{:#?}\n{:#?}\n{:#?}",
      timeseries.samples, timeseries.histograms, timeseries.exemplars
    );
...
pub async fn run() {
  ...
  let app: Router = Router::new()
    .route("/receive", post(handler_receive_prometheus_remote_write))
